...
class StatusRepository extends DbRepository
{
    ...
    {
        // statusテーブル全てとuserテーブルのuse_nameから
        // statusテーブルにuserテーブルのuser_nameを
        // statusテーブルのuser_idとuserテーブルのidが等しいレコードに追加し、
        // userテーブルのidと$user_idが等しい物を抽出し、
        // statusテーブルのcreated_atを基準に降順に取得する 
        $sql = "
            SELECT a.*, u.user_name
                FROM status a
                    LEFT JOIN user u ON a.user_id = u.id
